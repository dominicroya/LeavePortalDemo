// Test Scripts for Employee functions
...
public class EmployeeFunctions extends BaseClass{
...
//____Validation of Employement Details		
		Ast.assertEquals(hp.Empname.getText(), "Leave portal Employee");
		Ast.assertEquals(hp.mailID.getText(), "leaveportalemployee@datatemplate.com");
		Ast.assertEquals(hp.Joining_Date.getText(), "01/01/2019");
		Ast.assertEquals(hp.Date_of_Confirmation.getText(), "01/07/2019");
...
//____Validation of Leave Balance	
		//String l=hp.Leave_Balance.getText();
		float Num_leave_Balance=Float.parseFloat(hp.Leave_Balance.getText());
		...
		Ast.assertEquals(Float.parseFloat(hp.Leave_Balance.getText()), Num_leave_Balance);
...
//______Validation for presence of leavePolicy popup	
			Ast.assertEquals(true, lpc.Popupleavepolicy.isDisplayed());
...
//______Validation for presence of Holiday List popup
			Ast.assertEquals(true, hl.PopupHolidayList.isDisplayed());
		hl.closeHolidayList();
			Thread.sleep(3000);		
	}
...
//______Validation for status of leave after applying				
				Ast.assertEquals(lh.LeaveStatusAfterApply.getText(), "PendingApproval");
		}
...
//______Validation for status of leave after Cancelling				
		Ast.assertEquals(lh.LeaveStatusAfterCancel.getText(), "Cancelled");
		}
...
//______Validation for successful popup message	for change password
		Ast.assertEquals(true, chPwd.PopupPwdSuccessful.isDisplayed());
	}
...
//______Validation of Error message for change password     		 //Failed TC 
	...
	public void ApplyLeaveInvalidDate() throws InterruptedException
	{
		...
		lh.SubmitAppliedLeave();	
				Thread.sleep(5000);
//______Validation Error message for invalid dates for leave		//Failed TC 		
